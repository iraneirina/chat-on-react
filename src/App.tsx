...
export const App: FC = () => {
    ...
    const deleteChat = (chatId: string) => {
        setChats((chats) => {
            return chats.filter((item) => { return chatId !== item.id})
        })
        // setChats(chats.filter((item) => {return item.id !== chatId}))
    }
    ...
    return (
        <Routes>
            <Route path="/" element={<Header />} >
                ...
                <Route path="chats">
                    <Route index element={<ChatList chats={chats} onAddChat={onAddChat} deleteChat={deleteChat}/>} />
                    <Route path=":chatId"
                           element={<ChatPage
                               chats={chats}
                               onAddChat={onAddChat}
